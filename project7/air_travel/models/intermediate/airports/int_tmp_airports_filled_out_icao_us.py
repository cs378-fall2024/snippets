...
def model(dbt, session):
    ...
    input_df = dbt.ref("int_tmp_airports_missing_icao_us")
        
    # since we didn't snapshot int_tmp_airport_reviews, 
    ...
    if num_airports == 0:
        ...
        return session.createDataFrame(data = [], schema = ("name: string, icao: string, iata: string, state: string"))
